...
const Archive = () => {
  ...
  const fetchData = async () => {
      const response = await axios.get("http://localhost:3502/achive").catch(e => console.log(e));
                                          ...
                                          <thead className="MainTableThead">
                                                  <tr className="MainTableTr">
                                                      ...
                                                      {productsColumns.filter((p,i) => p["Header"] !== "id").map((column, index)=>(  
                                                          <th className="MainTableTh" key={index} style={{display : !column["show"] ? "none" : null, width:column["Header"] === "IMEI" ? 400 : 10}}>
                                                              {column["Header"]}
                                                              <button className='sortingArrowsBTN'>{isSorted ==='0' ?  (<UpArrow/>) : isSorted === '1' ? (<MainUpArrow/> ) :  (<DownArrow/>)  }</button>
                                                          </th>
                                                      ))}
                                                      <th className="TableMainTd">
                                                      <span className='archiveSpan'>تحویل</span>
                                                      </th>
                                                  </tr>
                                          </thead>
                                          <tbody>
                                              {posts.map((post, index) => (       
                                                  <tr  key={index}>
                                                      ...
                                                      <td className="TableMainTd" >
                                                          <span className='archiveSpan'>حضوری</span>
                                                      </td>
                                                  </tr>
                                              ))}
                                          </tbody>       
