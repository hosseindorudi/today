...
const Delivery = () => {
  const [columnSideBar, setColumnSideBar] = useState(false);
    ...
    const [postModalValue, setpostmodalValue] = useState(false);
    const [presenceModalValue, setPresencemodalValue] = useState(false);
  ...
  return (
            ...
            <>
                {productsColumns.length && 
                    <>
                        {postModalValue && <PostModal rowValus={rowValus} setpostmodalValue={setpostmodalValue}/>}
                        {presenceModalValue && <PresenceModal rowValus={rowValus} setPresencemodalValue={setPresencemodalValue}/>}
                                            ...
                                            <thead className="MainTableThead">
                                                    <tr className="MainTableTr">
                                                        ...
                                                        {productsColumns.filter((p,i) => p["Header"] !== "id").map((column, index)=>(  
                                                            <th className="MainTableTh" key={index} style={{display : !column["show"] ? "none" : null, marginRight: column["Header"] === "IMEI" ? 100 : 10, marginLeft: column["Header"] === "IMEI" ? 100 : 10}}>
                                                                {column["Header"]}
                                                    ...
                                                    <tr  key={index}>
                                                        ...
                                                        <td className="TableMainTd">
                                                            <div className='DivDelivery'>
                                                                <button className='PresenceDelivery' onClick={()=> setPresencemodalValue(true)}>حضوری</button>
                                                                <button className='PostDelivery' onClick={()=> setpostmodalValue(true)}>مرسولات</button>
